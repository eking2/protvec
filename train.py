...
from dataset import PreprocessVocab, ProtVecVocab, concat_seqs
...
def train_negsamp(model, loader, optimizer, neg_samples):
    ...
    for batch_idx, (centers, contexts) in enumerate(loader):
        ...
        # embeddings
        v, u_pos = model(centers, contexts)
        u_neg = model.sample_neg(n_contexts, neg_samples).to(device)

        # neg sample loss
        loss = model.neg_sample_loss(v, u_pos, u_neg)
...
if __name__ == '__main__':
    ...
    if args.corpus:
        PreprocessVocab(args.corpus, k=args.ngram, min_freq=args.min, window=args.context).run_all()

    if args.train:

        dataset = ProtVecVocab()
        loader = DataLoader(dataset, batch_size=args.batch, shuffle=True, collate_fn=concat_seqs)
        vocab_size = len(dataset.idx2word)

        model = skip_gram(vocab_size, args.embed, device).to(device)
        ...
        for epoch in pbar:
            ...
            loss = train_negsamp(model, loader, optimizer, args.neg)
